          ...
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {displayReviews.map((review, i) => (
              <motion.div
                ...
                className="break-inside-avoid bg-card rounded-2xl p-6 shadow-md border border-border hover:shadow-xl transition-shadow"
              >
                <div className="flex items-center gap-1 mb-3">
                  {Array.from({ length: 5 }).map((_, j) => (
                    <Star
                      key={j}
                      size={16}
                      className={j < review.rating ? "fill-accent text-accent" : "text-muted"}
                    />
                  ))}
                </div>
